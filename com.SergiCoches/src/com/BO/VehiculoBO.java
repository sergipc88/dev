...
import com.SergiCoches.VehiculoSiniestro;
...
public class VehiculoBO {
	...
	public List<List> consultaConPerdida() {
		Vehiculo v = new Vehiculo();
		Siniestro s = new Siniestro();
		Session session = HibernateUtil.getSessionFactory().openSession();
	...
	public List<VehiculoSiniestro> consultaConPerdida2() {
		Vehiculo v = new Vehiculo();
		Siniestro s = new Siniestro();
		Session session = HibernateUtil.getSessionFactory().openSession();
		String queryString = "select new com.SergiCoches.VehiculoSiniestro(v, s.perdida) from Vehiculo v join v.siniestros s where s.perdida > ?";
		Query hqlQuery2 = session.createQuery(queryString).setParameter(0, 1000);
		List lista = hqlQuery2.list();
